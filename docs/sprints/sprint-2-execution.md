...
- Next action: retain licensed playback comparison as the remaining empirical entity-state pre-release validation task
...
- 2026-07-17: Replaced vague external follow-up with concrete UntitledParser framing and licensed-playback acceptance procedures. The published UntitledParser binary was incompatible with the arm64 container, and licensed game/build/map assets were absent; internal decoder or detector work could not honestly substitute for these checks.
...
| G10         | worktree | `pnpm --filter @l4dstats/detectors exec vitest run --maxWorkers=1` |    0 | observations `ec346c38…`; evidence `58d7af84…`                | real demo `299019bf…`, 138,696 complete aim samples | none                            | deterministic real no-finding artifact |
| performance | worktree | `pnpm --filter @l4dstats/detectors benchmark`                      |    0 | 100,000 aim samples mean 82.46 ms                             | synthetic benchmark                                 | none                            | passed                                 |
...
| telemetry   | worktree | `pnpm --filter @l4dstats/l4d2-schema verify:corpus`                |    0 | 10 demos; 92 epochs; 104,183 frames; 932,553 observations     | ten quarantined protocol-2100 SourceTV demos        | none                            | passed                                 |
| identity    | worktree | `pnpm --filter @l4dstats/l4d2-schema verify:corpus`                |    0 | 76 privacy-bound human epochs; no raw identifiers             | timed updates and safe snapshot reconciliation      | pseudonym key                   | passed                                 |
...
| framing     | worktree | `scripts/run-untitled-parser-check.sh`                             |    0 | `sprint-1-independent-framing.json`; 22/22, 231,337 commands  | pinned UntitledParser plus quarantined real demos   | .NET 7 SDK                      | passed                                 |
| playback    | external | procedure in `sprint-1-playback-validation.md`                     |      | pending                                                       | licensed L4D2 player-state capture                  | game, build and matching assets | does not block Sprint 3 development    |

The framing portion of the pre-release gate subsequently passed on 2026-07-18:
the pinned AnyCPU UntitledParser source built under .NET 7.0.410 for Linux
arm64, and all 22 demos matched across 231,337 commands. See
`sprint-1-independent-framing.json`. Licensed player-state playback remains
pending because neither the game nor matching assets are available here.
