...
function terminalBoundaryFromLiveScores(value: DemoStats): number[] | null {
  ...
  if (!timeline?.length || !opening || opening.length < 2) return null;
  const liveMax = [0, 0];
  for (const point of timeline)
    for (let team = 0; team < 2; team++)
      liveMax[team] = Math.max(liveMax[team]!, point.survivorScores[team] ?? 0);
...
export function reconstructVersusScores(
  ...
  stats.forEach((value, index) => {
    ...
    if (!terminal || terminal.length < 2) return;
    ...
    const nextOpening = adjacentChapter
      ...
      : undefined;
    const confirmedByNext = Boolean(nextOpening && nextOpening.length >= 2);
    ...
    const recoveredFromLiveScore = Boolean(
      liveBoundary?.some((score, team) => score > (terminal[team] ?? 0)),
    );
    const boundary = confirmedByNext
      ...
      : liveBoundary
        ...
        : terminal;
    const teamA = boundary[0] ?? 0;
    const teamB = complete ? (boundary[1] ?? 0) : null;
    rows.push({
